<template>
    <div class="flex justify-end">
        <!-- InstructorBox toggle button -->
        <input id="instructor" v-model="getInstructorName" @click="toogleBox"
        ...
        <!-- InstructorBox content -->
        <Transition name="box" mode="in-out">
            <form ref="formInstructor" v-show="openBox" @submit.prevent="processing" novalidate
                ...
                <input v-model="instructorData.name"
                    ...
                    type="text" placeholder="Nome do Instrutor *" required />

                <input v-model="instructorData.email"
                    ...
                    type="text" placeholder="E-mail do Instrutor *" required />
...
const emit = defineEmits("send-instructor-data");

const openBox = ref(false);
const formInstructor = ref("");
const getInstructorName = ref(null);
...
const toogleBox = () => {
    ...
    formInstructor.value.classList.remove("was-validated");
};

const processing = () => {
    if (formInstructor.value.checkValidity() === true) {
        formInstructor.value.classList.remove("was-validated");
        emit('send-instructor-data', instructorData.value);
        getInstructorName.value = instructorData.value.name;
        toogleBox();
        instructorData.value.name = null;
        instructorData.value.email = null;
    } else {
        formInstructor.value.classList.add("was-validated");
    }
}

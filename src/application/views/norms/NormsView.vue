<template>
    <main class="container mx-auto mt-8">
        <h1 class="font-bold text-xl">Normas e Regulamentações</h1>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">Normas</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="mt-4 md:flex md:items-center md:justify-between">
            <div class="flex gap-x-4">
                <button @click="showDialogNewNorm" class="btn-primary">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M12 9v6m3-3H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span class="font-semibold">Nova Norma</span>
                </button>

                <RouterLink :to="{ name: 'systems' }" class="btn-light">
                    <i class="bi bi-journal-text text-[1rem]"></i>
                    <span class="font-semibold">Sistemas</span>
                </RouterLink>
            </div>

            <div class="flex gap-x-3 relative">
                <button class="text-sm btn-light w-[155px]" @click="toggleDropdown">
                    {{ normsStatus }} <i class="bi bi-chevron-down"></i>
                </button>
                <Transition name="dropdown" mode="out-in">
                    <div v-if="open_dropdown"
                        class="absolute dropdown bg-white shadow-xl py-1 top-[45px] left-0 z-[2] rounded-lg w-[155px]">
                        <ul class="flex flex-col">
                            <li class="px-4 text-sm cursor-pointer hover:bg-blue-50 py-3" @click="setNormsStatus('ALL')">
                                Todos os estados</li>
                            <hr class="border-slate-300 my-0" />
                            <li class="px-4 text-sm cursor-pointer hover:bg-blue-50 py-3" @click="setNormsStatus('ACTIVE')">
                                Ativas</li>
                            <hr class="border-slate-300 my-0" />
                            <li @click="setNormsStatus('INACTIVE')"
        ...
        <div class="my-6 sm:flex sm:items-center sm:justify-between ">
            <div class="text-sm text-gray-500 dark:text-gray-400">
                Página <span class="font-normal text-gray-700 dark:text-gray-100">1 of 10</span>
            </div>
    ...
    <BaseModalDialog @close-modal="closeModalAlertDelete" @yes="axiosDeleteNorm" @no="closeModalAlertDelete"
        :open-modal="open_modal_delete" :enable-close-outside="true" title="Eliminar Norma" type="DANGER" buttons="YES_NO">
        Você está prestes a eliminar esta<strong> Norma.</strong> Deseja realmente
        prosseguir? *Essa operação não pode ser desfeita.
    </BaseModalDialog>

    <BaseModalContent @close-modal="closeModalPreviewNorm" :open-modal="open_modal_preview_norm"
        ...
        <div class="relative max-w-[750px] w-full">
            <div class="bg-white w-full rounded-t-lg">
                <div class="bg-gray-50 border-b rounded-t-lg pb-2 px-8">
                    <button @click="closeModalPreviewNorm" :desabled="processing"
                    ...
                    <div class="text-center pt-4 pb-2">
                        <h2 class="font-semibold text-2xl text-gray-500">Norma e Regulamentação</h2>
                        <span class="text-gray-400">Informação detalhada sobre a norma.</span>
                    </div>
                </div>
            ...
            <div class="bg-gray-50 border-t rounded-b-lg p-4">
                <div class="flex items-center justify-end gap-x-3">
                    ...
                    <button type="button" @click="closeModalPreviewNorm" :desabled="processing"
    ...
    <BaseModalContent @close-modal="closeModalNewNorm" :open-modal="open_modal_new_norm" :enable-close-outside="false">
        ...
        <div class="relative max-w-[800px] w-[90%]">
            <div class="bg-white w-full rounded-t-lg overflow-hidden">
                <div class="bg-gray-50 border-b pb-2 px-8">
                    <button @click="closeModalNewNorm" :desabled="processing"
                        class="absolute -top-3 -right-3 flex items-center justify-center w-1/2 p-2.5 text-sm tracking-wide transition-colors duration-200 bg-gray-50 border shadow-sm rounded-lg shrink-0 sm:w-auto gap-x-2 hover:bg-gray-100">
                        <i class="bi bi-x-lg text-[1rem]"></i>
                    </button>
                    <div class="text-center pt-4 pb-2">
                        <h2 class="font-semibold text-2xl text-gray-500">Norma e Regulamentação</h2>
                        <span class="text-gray-400">Cadastre e gerencie normas de forma intuitiva</span>
                    </div>
                </div>

                <div class="px-6 overflow-y-auto h-[70vh] beauty-scrollbar">
                    <FormNorms @processing="processing = true" @success="stop" @error="stop" @success-update="stop"
...
import NormStatus from '@/components/norms/NormStatus.vue';
...
import PreviewNormBox from '@/components/norms/PreviewNormBox.vue';
//import PageLoader from "@/components/PageLoader.vue";


const searchBy = ref("");
const norm_id_to_actiion = ref(null);
const open_dropdown = ref(false);
const open_modal_delete = ref(false);
const open_modal_new_norm = ref(false);
const open_modal_preview_norm = ref(false);
const normsStatus = ref('Todos os estados');


const setNormsStatus = (state) => {
    searchBy.value = '';

    if (state === 'ACTIVE') {
        normsStatus.value = 'Ativas';
    } else if (state === 'INACTIVE') {
        normsStatus.value = 'Não Ativas';
    } else if (state === 'REVISION') {
        normsStatus.value = 'Revisão';
    } else {
        normsStatus.value = 'Todos os estados';
    }
...
const showDialogNewNorm = () => {
    open_modal_new_norm.value = true;
    document.body.style.overflow = "hidden";
};

const showDialogPreviewNorm = (norm) => {
    norm_id_to_actiion.value = norm;
    open_modal_preview_norm.value = true;
    document.body.style.overflow = "hidden";
};

const showDialogDelete = (norm_id) => {
    open_modal_delete.value = true;
    norm_id_to_actiion.value = norm_id;
    document.body.style.overflow = "hidden";
};
...
const closeModalNewNorm = () => {
    open_modal_new_norm.value = false;
    document.body.style.overflow = "auto";
};

const closeModalPreviewNorm = () => {
    open_modal_preview_norm.value = false;
    document.body.style.overflow = "auto";
};
...
onMounted(() => {
    ...
    document.title = "Comply 360 | Normas e Regulamentações";

<template>
    <main class="container mx-auto mt-8">
        <h1 class="font-bold text-xl">Riscos Potenciais</h1>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span
                            class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">Riscos</span>
                    </div>
                </li>
            </ol>
        </nav>
                    ...
                    <div v-if="open_dropdown"
                        ...
                        <ul class="flex flex-col">
                            <li class="px-4 text-sm cursor-pointer hover:bg-blue-50 py-3"
                                @click="setRisksStatus('ALL')">
                                Todos os estados</li>
                            <hr class="border-slate-300 my-0" />
                            <li class="px-4 text-sm cursor-pointer hover:bg-blue-50 py-3"
                                @click="setRisksStatus('ACTIVE')">
                                Ativas</li>
                            <hr class="border-slate-300 my-0" />
                            <li @click="setRisksStatus('INACTIVE')"
    ...
    <BaseModalDialog @close-modal="closeModalAlertDelete" @yes="axiosDeleteNorm" @no="closeModalAlertDelete"
        :open-modal="open_modal_delete" :enable-close-outside="true" title="Eliminar Risco" type="DANGER"
        buttons="YES_NO">
        Você está prestes a eliminar este<strong> Risco Potencial.</strong> Deseja realmente
        prosseguir? *Essa operação não pode ser desfeita.
    </BaseModalDialog>

    <BaseModalContent @close-modal="closeModalPreviewRisk" :open-modal="open_modal_preview_risk"
        ...
        <div class="relative max-w-[750px] w-full">
            <div class="bg-white w-full rounded-t-lg">
                <div class="bg-gray-50 border-b rounded-t-lg pb-2 px-8">
                    <button @click="closeModalPreviewRisk" :desabled="processing"
                    ...
                    <div class="text-center pt-4 pb-2">
                        <h2 class="font-semibold text-2xl text-gray-500">Risco Potencial</h2>
                        <span class="text-gray-400">Informação detalhada sobre o Risco.</span>
                    </div>
                </div>
            ...
            <div class="bg-gray-50 border-t rounded-b-lg p-4">
                <div class="flex items-center justify-end gap-x-3">
                    ...
                    <button type="button" @click="closeModalPreviewRisk" :desabled="processing"
    ...
    <BaseModalContent @close-modal="closeModalNewRisk" :open-modal="open_modal_new_risk" :enable-close-outside="false">
        ...
        <div class="relative max-w-[800px] w-[90%]">
            <div class="bg-white w-full rounded-t-lg overflow-hidden">
                <div class="bg-gray-50 border-b pb-2 px-8">
                    <button @click="closeModalNewRisk" :desabled="processing"
                        class="absolute -top-3 -right-3 flex items-center justify-center w-1/2 p-2.5 text-sm tracking-wide transition-colors duration-200 bg-gray-50 border shadow-sm rounded-lg shrink-0 sm:w-auto gap-x-2 hover:bg-gray-100">
                        <i class="bi bi-x-lg text-[1rem]"></i>
                    </button>
                    <div class="text-center pt-4 pb-2">
                        <h2 class="font-semibold text-2xl text-gray-500">Risco Potencial</h2>
                        <p class="text-gray-400 max-w-[600px] mx-auto">Identifique, avalie, e gerencie potenciais ameaças que podem afetar a conformidade da organização</p>
                    </div>
                </div>

                <div class="px-6 overflow-y-auto h-[70vh] beauty-scrollbar">
                    <FormRisk @processing="processing = true" @success="stop" @error="stop" @success-update="stop"
                        @error-update="stop" @close-modal="closeModalNewRisk" />
                </div>
            </div>
            ...
            <div class="bg-gray-50 border-t rounded-b-lg p-4">
                <div class="flex items-center justify-end gap-x-3">
                    <button @click="saveOportunity" :desabled="processing" class="btn-primary">
                        ...
                        <span class="font-semibold">Salvar Risco</span>
                    </button>

                    <button type="button" @click="closeModalNewRisk" :desabled="processing"
                        class="flex items-center justify-center w-1/2 px-5 py-2 text-sm tracking-wide transition-colors duration-200 bg-gray-200 rounded-lg shrink-0 sm:w-auto gap-x-2 hover:bg-gray-300">
                        <i class="bi bi-x-circle"></i>
                        <span class="font-semibold">Cancelar</span>
                    </button>
...
import FormRisk from '@/components/process/risks/FormRisk.vue';
...
import PreviewRiskBox from '@/components/process/risks/PreviewRiskBox.vue';
//import PageLoader from "@/components/PageLoader.vue";


const searchBy = ref("");
const risk_id_to_actiion = ref(null);
const open_dropdown = ref(false);
const open_modal_delete = ref(false);
const open_modal_new_risk = ref(false);
const open_modal_preview_risk = ref(false);
const risksStatus = ref('Todos os estados');


const setRisksStatus = (state) => {
    searchBy.value = '';

    if (state === 'ACTIVE') {
        risksStatus.value = 'Ativas';
    } else if (state === 'INACTIVE') {
        risksStatus.value = 'Não Ativas';
    } else if (state === 'REVISION') {
        risksStatus.value = 'Revisão';
    } else {
        risksStatus.value = 'Todos os estados';
    }
...
const showDialogNewOportunity = () => {
    open_modal_new_risk.value = true;
    document.body.style.overflow = "hidden";
};

const showDialogPreviewOportunity = (oportunity) => {
    risk_id_to_actiion.value = oportunity;
    open_modal_preview_risk.value = true;
    document.body.style.overflow = "hidden";
};

const showDialogDelete = (id) => {
    open_modal_delete.value = true;
    risk_id_to_actiion.value = id;
    document.body.style.overflow = "hidden";
};
...
const closeModalNewRisk = () => {
    open_modal_new_risk.value = false;
    document.body.style.overflow = "auto";
};

const closeModalPreviewRisk = () => {
    open_modal_preview_risk.value = false;
    document.body.style.overflow = "auto";
};
...
onMounted(() => {
    document.title = "Comply 360 | Riscos Potenciais";
});

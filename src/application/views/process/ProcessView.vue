<template>
    <main class="container mx-auto my-8">
        <h1 class="font-bold text-xl">Gerenciamento de Processos</h1>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">Processos</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="mt-5 md:flex md:items-center md:justify-between">
            <div class="options flex gap-x-2 items-center">
                ...
                <button @click="showDialogNewProcess" class="btn-primary">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M12 9v6m3-3H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span class="font-semibold">Novo Processo</span>
                </button>
                <RouterLink :to="{ name: 'oportunities' }" class="btn-light">
                    <i class="bi bi-ui-checks-grid text-[1rem]"></i>
                    <span class="font-semibold">Oportunidades</span>
                </RouterLink>
                <RouterLink :to="{ name: 'risks' }" class="btn-light">
                    <i class="bi bi-exclamation-triangle text-[1rem]"></i>
                    <span class="font-semibold">Riscos</span>
                </RouterLink>
            </div>

            <div class="flex gap-x-3 relative">
                <button class="text-sm btn-light w-[155px]" @click="toggleDropdown">
                    {{ processState }} <i class="bi bi-chevron-down"></i>
                </button>
                    ...
                    <div v-if="open_dropdown"
                        ...
                        <ul class="flex flex-col">
                            <li class="px-4 text-sm cursor-pointer hover:bg-blue-50 py-3"
                                @click="setProcessState('ALL')">
                                Todos os estados</li>
                            <hr class="border-slate-300 my-0" />
                            <li class="px-4 text-sm cursor-pointer hover:bg-blue-50 py-3"
                                @click="setProcessState('DONE')">
                                Concluídos</li>
                            <hr class="border-slate-300 my-0" />
                            <li class="px-4 text-sm cursor-pointer hover:bg-blue-50 py-3"
                                @click="setProcessState('IN_REVISION')"> Em Revisão
                            </li>
                            <hr class="border-slate-300 my-0" />
                            <li @click="setProcessState('IN_PROGRESS')"
                                class="px-4 text-sm cursor-pointer hover:bg-blue-50 py-3">Em Progresso
                            </li>
                        </ul>
                    </div>
                </Transition>
        ...
        <div class="grid grid-cols-2 gap-x-4 my-7">
            <ProcessStatisticSection />
                            ...
                            <tbody
                                ...
                                <tr>
                                    ...
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <p class="text-sm font-normal table-long-text text-gray-600">Processo de
                                            Controle de Qualidade</p>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <p class="text-sm font-normal table-long-text text-gray-600">ISO 47001
                                        </p>
                                    </td>

                                    <td class="px-4 py-4 text-sm whitespace-nowrap">
                                        <div>
                                            <p class="font-medium text-gray-800 dark:text-white leading-none">Naor de
                                                Almeida</p>
                                            <span class="text-sm font-normal text-blue-600">Administrador
                                            </span>
                                        </div>
                                    </td>
        ...
        <div class="my-6 sm:flex sm:items-center sm:justify-between ">
            <div class="text-sm text-gray-500">
                Página <span class="font-Processol text-gray-700 dark:text-gray-100">1 de 10</span>
            </div>
    ...
    <BaseModalDialog @close-modal="closeModalAlertDelete" @yes="axiosDeleteProcess" @no="closeModalAlertDelete"
        :open-modal="open_modal_delete" :enable-close-outside="true" title="Eliminar Processo" type="DANGER"
        buttons="YES_NO">
        Você está prestes a eliminar este<strong> Processo e todos os dadso associados oa mesmo.</strong> Deseja
        realmente
        prosseguir? *Essa operação não pode ser desfeita.
    </BaseModalDialog>

    <BaseModalContent @close-modal="closeModalAlertNewProcess" :open-modal="open_modal_new_process"
        ...
        <div class="relative max-w-[800px] w-full">
            <div class="w-full bg-white rounded-t-lg h-[80vh] overflow-hidden">
                <div class="bg-gray-50 border-b pb-2">
                    <button @click="closeModalAlertNewProcess"
                    ...
                    <div class="text-center pt-4 pb-2 px-8">
                        <h2 class="font-semibold text-2xl text-gray-500">Processo</h2>
                        <span class="text-gray-400">Crie, monitore e rastreie processos relacionados
                            às normas e programas.</span>
                    </div>
                </div>

                <div class="h-[68vh] overflow-auto beauty-scrollbar px-8">
                    <FormProcess @processing="processing = true" @success="stop" @error="stop"
                        @close-modal="closeModalAlertNewProcess" />
                </div>
            </div>
            ...
            <div class="bg-gray-50 border-t rounded-b-lg p-4">
                <div class="flex items-center justify-end gap-x-3">
                    <button class="btn-primary" :disabled="processing">
                        ...
                        <span class="font-semibold">Salvar Processo</span>
                    </button>
                    <button type="button" @click="closeModalAlertNewProcess"
    ...
    <PageLoader :open="show_delectation_process" text="Processando..." />
</template>

<script setup>
import { onMounted, ref } from 'vue';
import ProcessIndicatorsSection from '@/components/process/ProcessIndicatorsSection.vue';
import ProcessStatus from '@/components/process/ProcessStatus.vue';
import ProcessStatisticSection from '@/components/process/ProcessStatisticSection.vue';
...
const processState = ref("Todos os estados");
const searchBy = ref("");
const processing = ref(false);
const open_dropdown = ref(false);
const show_delectation_process = ref(false);
const open_modal_delete = ref(false);
const open_modal_new_process = ref(false);
...
const showDialogNewProcess = () => {
    open_modal_new_process.value = true;
    document.body.style.overflow = "hidden";
};

const closeModalAlertNewProcess = () => {
    open_modal_new_process.value = false;
    document.body.style.overflow = "auto";
};

const setProcessState = (state) => {
    searchBy.value = '';

    if (state === 'DONE') {
        processState.value = 'Concluídos';
    } else if (state === 'IN_REVISION') {
        processState.value = 'Em Revisão';
    } else if (state === 'IN_PROGRESS') {
        processState.value = 'Em Progresso';
    } else {
        processState.value = 'Todos os estados';
    }
...
onMounted(() => {
    document.title = "Comply 360 | Gerenciamento de Processos";
});

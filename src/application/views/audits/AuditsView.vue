<template>
    <main class="container mx-auto mt-8">
        <h1 class="font-bold text-xl">Auditorias e inspeções</h1>
        ...
        <div class="mt-5 md:flex md:items-center md:justify-between">
            <button @click="showDialogNewAudit" class="btn-primary">
                <i class="bi bi-calendar-plus text-[1rem]"></i>
                <span class="font-semibold">Agendar Auditoria</span>
            </button>

            <div class="flex gap-x-3 relative">
                <button class="text-sm btn-light w-[155px]" @click="toggleDropdown">
                    {{ auditsStatus }} <i class="bi bi-chevron-down"></i>
                </button>
                ...
                <Transition name="dropdown" mode="out-in">
                    <div v-if="open_dropdown"
                        ...
                        <ul class="flex flex-col">
                            ...
                            <li class="px-4 text-sm cursor-pointer hover:bg-blue-50 py-3"
                                @click="setAuditStatus('OPENED')">
                                Abertas</li>
                            <hr class="border-slate-300 my-0" />
                            <li class="px-4 text-sm cursor-pointer hover:bg-blue-50 py-3"
                                @click="setAuditStatus('RESOLVED')">
                                Resolvidas</li>
                            <hr class="border-slate-300 my-0" />
                            <li @click="setAuditStatus('IN_COURSE')"
        ...
        <AuditsStatistics auditsData="" />

        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 mt-7">
            <thead class="bg-gray-50 dark:bg-gray-800">
                <tr>
                    <th
                        class="py-3.5 px-4 text-sm font-medium text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        <div class="flex items-center gap-x-3">
                            <input @change="onChangeSelectedAllAudits" v-model="selected_all_audits" type="checkbox"
                            ...
                            <button @click="changeOrderId()"
                                ...
                                <i v-if="!order_id" class="bi bi-sort-numeric-down-alt h-5 text-[1.2rem]"></i>
                            </button>
                        </div>
                    </th>
                    <th scope="col"
                        class="py-3.5 px-2 text-sm font-medium text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tr>
                    <td class="px-4 py-4 text-sm font-medium text-gray-700 dark:text-gray-200 whitespace-nowrap">
                        <div class="inline-flex items-center gap-x-3">
                            <input id="unconformity.id" @change="onChangeSelectAudit" :checked="selected_all_audits"
                                type="checkbox"
                                class="text-blue-500 border-gray-300 rounded dark:bg-gray-900 dark:ring-offset-gray-900 dark:border-gray-700">
                            <span># 12</span>
                        </div>
                    ...
                    <td class="px-2 py-4 text-sm whitespace-nowrap">
                        <AuditStatus status="SUSPENDED" />
                    </td>
                    <td class="px-2 py-4 text-sm whitespace-nowrap relative">
                        <BaseDropdownOptions :options="['DELETE', 'PREVIEW_ROUTER_LINK']"
                            :preview_route_object="{ name: 'audit-preview', params: { id: 2 } }"
        ...
        <div class="my-6 sm:flex sm:items-center sm:justify-between ">
            <div class="text-sm text-gray-500 dark:text-gray-400">
                Página <span class="font-Auditorial text-gray-700 dark:text-gray-100">1 de 10</span>
            </div>
    ...
    <BaseModalDialog @close-modal="closeModalAlertDelete" @yes="axiosDeleteDocumentation" @no="closeModalAlertDelete"
        ...
        buttons="YES_NO">
        Você está prestes a eliminar esta<strong> Auditoria.</strong> Deseja realmente
        prosseguir? *Essa operação não pode ser desfeita.
    </BaseModalDialog>

    <BaseModalContent @close-modal="closeModalNewAudit" :open-modal="open_modal_new_audit"
        ...
        <div class="relative max-w-[800px] w-[90%]">
            <div class="bg-white w-full rounded-t-lg overflow-hidden">
                <div class="bg-gray-50 border-b pb-2 px-8">
                    <button @click="closeModalNewAudit" :desabled="processing"
                    ...
                    <div class="text-center pt-4 pb-2">
                        <h2 class="font-semibold text-2xl text-gray-500">Auditoria e Inspeção</h2>
                        <span class="text-gray-400"> Agende auditorias, rastreie resultados,
                            assegurando a conformidade e a melhoria contínua.</span>
                    </div>
                </div>

                <div class="px-6 overflow-y-auto h-[70vh] beauty-scrollbar">
                    <FormAudit @processing="processing = true" @success="stop" @error="stop" @success-update="stop"
                        @error-update="stop" @close-modal="closeModalNewNorm" />
                </div>
            </div>
            ...
            <div class="bg-gray-50 border-t rounded-b-lg p-4">
                <div class="flex items-center justify-end gap-x-3">
                    <button @click="saveNorm" :desabled="processing" class="btn-primary">
                        ...
                        <span class="font-semibold">Salvar Norma</span>
                    </button>

                    <button type="button" @click="closeModalNewNorm" :desabled="processing"
...
import AuditsStatistics from "@/components/audits/AuditsStatistics.vue";
import FormAudit from "@/components/audits/FormAudit.vue";
import FormNorms from '@/components/norms/FormNorms.vue';
import PageLoader from "@/components/PageLoader.vue";


const searchBy = ref("");
const open_dropdown = ref(false);
const id_audit_to_delete = ref(false);
const show_deletation_process = ref(false);
const open_modal_delete = ref(false);
const selected_all_audits = ref(false);
const at_least_one_audit_selected = ref(null);
const count_selected_audit = ref(null);
const auditsStatus = ref('Todos os estados');

const open_modal_new_audit = ref(false);

const list_audit_to_delete = ref([]);
const auditsData = ref([]);


const setAuditStatus = (state) => {
    searchBy.value = '';

    if (state === 'DONE') {
        auditsStatus.value = 'Concluídas';
    } else if (state === 'SUSPENDED') {
        auditsStatus.value = 'Suspensa';
    } else if (state === 'IN_COURSE') {
        auditsStatus.value = 'Em Progresso';
    } else {
        auditsStatus.value = 'Todos os estados';
    }
...
const onChangeSelectedAllAudits = (event) => {
    if (event.target.checked) {
        count_selected_audit.value = auditsData.value.length;
        at_least_one_audit_selected.value = true;

        list_audit_to_delete.value = [];
        auditsData.value.forEach((documentation) => {
            list_audit_to_delete.value.push({ id: documentation.id })
        });
    }
    else {
        at_least_one_audit_selected.value = false;
        list_audit_to_delete.value = [];
    }
};

const onChangeSelectAudit = (event) => {
    if (event.target.checked) {
        count_selected_audit.value += 1;
        list_audit_to_delete.value.push({ id: Number.parseInt(event.target.id) })
    }
    else {
        count_selected_audit.value -= 1;
        list_audit_to_delete.value = list_audit_to_delete.value.filter((obj => obj.id !== Number.parseInt(event.target.id)));
    }

    if (count_selected_audit.value > 0) {
        if (count_selected_audit.value === auditsData.value.length) {
            selected_all_audits.value = true;
        }

        at_least_one_audit_selected.value = true;
    }
    else {
        selected_all_audits.value = false;
        at_least_one_audit_selected.value = false;
    }
};
...
const showDialogDelete = (norm_id) => {
    ...
    id_audit_to_delete.value = norm_id;
...
const showDialogNewAudit = () => {
    open_modal_new_audit.value = true;
    document.body.style.overflow = "hidden";
};

const closeModalNewAudit = () => {
    open_modal_new_audit.value = false;
    document.body.style.overflow = "auto";
};
...
onMounted(() => {
    document.title = "Comply 360 | Auditorias e inspeções";
});
